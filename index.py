...
async def receive_messages(ws, private_key, public_keys):
    while True:
        try:
            ...
            print(f"\n[{msg_data['from']}]: {decrypted}\n> ", end="")
        except:
            continue

# ---------------- Chat loop ---------------- #
async def chat_client():
    ws_url = input("[?] Enter WebSocket URL (wss://...): ").strip()
    ...
    async with websockets.connect(ws_url) as ws:
        ...
        print(f"[+] Waiting for {peer_username}'s public key...")
        while peer_username not in public_keys:
            await asyncio.sleep(0.5)
        print(f"[+] Peer public key received. You can start typing now. Messages go to {peer_username}.")

        # Main chat loop
        while True:
            msg_text = await ainput("> ")
